...
/// T-Head C906 custom PTE extension bits (bits 63:59).
///
/// Kernel (normal, cacheable) memory: C=1, B=1, SH=1, SE=1 → 0b01111
/// Device (MMIO, non-cacheable) memory: SO=1, SH=1, SE=1 → 0b10011
mod thead {
    const SO: u64 = 1 << 63;
    const C: u64  = 1 << 62;
    const B: u64  = 1 << 61;
    ...
    const KERNEL_FLAGS: u64 = C | B | SH | SE;
    const DEVICE_FLAGS: u64 = SO | SH | SE;

    use crate::MappingFlags;

    pub fn extension_bits(flags: MappingFlags) -> u64 {
...
impl GenericPTE for Rv64PTE {
    fn new_page(paddr: PhysAddr, flags: MappingFlags, _is_huge: bool) -> Self {
        let pte_flags = PTEFlags::from(flags) | PTEFlags::A | PTEFlags::D;
        debug_assert!(pte_flags.intersects(PTEFlags::R | PTEFlags::X));
        let ext = thead::extension_bits(flags);
        Self(pte_flags.bits() as u64 | ((paddr.as_usize() >> 2) as u64 & Self::PHYS_ADDR_MASK) | ext)
    }
    ...
    fn set_flags(&mut self, flags: MappingFlags, _is_huge: bool) {
        ...
        debug_assert!(pte_flags.intersects(PTEFlags::R | PTEFlags::X));
        let ext = thead::extension_bits(flags);
        self.0 = (self.0 & Self::PHYS_ADDR_MASK) | pte_flags.bits() as u64 | ext;
    }
