...
//!
//! 与 `host.rs` 配合：在 `dwc2_host_init` 之后由 `enumerate_root_port()` 调用；返回 Mass Storage 设备四元组供 M3/M3.5 使用。
...
use super::setup;
use crate::log::{usb_log_flush_residual, LineBufferedUsbLog};

/// USB `bDeviceClass`：Hub。
const USB_CLASS_HUB: u8 = 0x09;
/// QEMU 默认 `usb-hub`（插在根口与首个外设之间）VID/PID（与 `plan.md` / `host` 一致）。
...
fn visit_default_depth(
    ...
    if is_msc_candidate(iface_class, vid, pid) {
        let _ = dwc2_ep0::mass_storage_bulk_only_reset(u32::from(fn_addr), 0, ep0_mps);
        st.note_msc(vid, pid, ep0_mps, u32::from(fn_addr));
        write_indent(&mut w, depth);
        let _ = writeln!(
            w,
            "[USB]   -> Mass Storage candidate (BOT reset attempted)"
        );
    }
...
pub fn enumerate_bus_print_tree_only() -> UsbResult<()> {
    let mut w = LineBufferedUsbLog;
    let _ = writeln!(
        w,
        "[USB] topology: recursive hub scan (QEMU may insert virtual usb-hub on single root port)"
